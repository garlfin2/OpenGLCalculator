...
namespace glm
{
    ...
    /// The interpolation always take the short path when the spin _count is positive and long path
    /// when _count is negative. Rotation is performed at constant speed.
    ///
    /// @param x A quaternion
    /// @param y A quaternion
    /// @param a Interpolation factor. The interpolation is defined beyond the range [0, 1].
    /// @param k Additional spin _count. If Value is negative interpolation will be on "long" path.
    ///
    /// @tparam T A floating-point scalar type
